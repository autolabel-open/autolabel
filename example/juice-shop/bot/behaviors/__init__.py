...
async def choose_and_exec_behavior(
    current_state: States, attack: bool = False
) -> States:
    normal_behaviours = {
        b: info.weight
        for b, info in behavior_infos.items()
        if not info.is_attack and current_state in info.avaliable_states
    }
    attack_behaviours = {
        b: info.weight
        for b, info in behavior_infos.items()
        if info.is_attack and current_state in info.avaliable_states
    }
    if attack:
        avaliable_behaviors = attack_behaviours
    else:
        avaliable_behaviors = normal_behaviours
