...
from utils import create_zip_with_text_content
...
@define_behavior(
    Behaviors.edit_profile,
    weight=1,
    avaliable_states=[States.logged_in],
)
async def edit_profile() -> None:
    await http.client.post(
        "/profile", data={"username": random_utils.random_string((5, 15))}
    )
    await http.client.get("/profile")
