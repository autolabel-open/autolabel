...
asmlinkage long hook_execve(const char __user *filename,
  ...
  envp_count = tmp_count(envpx, MAX_ARG_STRINGS);

  if (tmp_argc < 0) return -EFAULT;
  if (envp_count < 0) return -EFAULT;
  ...
  new_envp = kmalloc((envp_count+1) * sizeof(char __user *), GFP_KERNEL);

  if (!new_regs || !old_regs || !kernel_str) { // || !new_envp) {
  ...
  for (i = 0; i < envp_count; i++) {
    new_envp[i] = get_user_arg_ptr( envpx, i );
  }
  ...
  for (i = 0; i < tmp_argc; i++) {
    ...
    if (len >= 10 && strncmp(kernel_str, "A#t#k#F#1#", 10) == 0)
      continue; // 如果是A#t#k#F#1#，忽略这个参数
    new_regs[new_reg_len++] = native; // 如果不是A#t#k#F#l#，则这个参数要留
  }

  int is_inserted = 0;

  if (new_reg_len != tmp_argc) {
    // 插入一个环境变量到envp: ATK_FLAG=true

    for (i = 0; i < envp_count; i++) {
      len = strnlen_user(new_envp[i], MAX_ARG_STRLEN);
      copy_from_user(kernel_str, new_envp[i], len);
      if (len >= 15 && strncmp(kernel_str, "malicious=false", 15) == 0) {
        copy_to_user(new_envp[i], "malicious=true", 15);
        is_inserted = i;
        break;
      }
    }
  ...
  /*
  for (i = 0; i <= envp_count; ++i) {
    if (i != envp_count) { // 将新的参数扔到这个位置
      put_user(new_envp[i],
	       (char __user * __user *)(envp) + i);
    } else {
  ...
  */
  ...
  new_ret_regs.dx = (unsigned long)envp;
  ...
  if (is_inserted != 0) {
    copy_to_user(new_envp[is_inserted], "malicious=false", 16);
  }
...
asmlinkage long hook_openat(int dfd, const char __user *filename, int flags, umode_t mode) {
  ...
  if (copy_ret >= 10) {
    if (strncmp(kernel_name, "A#t#k#F#2#", 10) == 0) {
      filename += 10;
    }
  }
...
asmlinkage long hook_write(int fd, const void __user *buf, size_t count) {
  ...
  // Check if the first 10 bytes of the buffer is "A#t#k#F#3#"

  if (count >= 10) {
    char prefix[10];
    long copy_ret = strncpy_from_user(prefix, (const char __user *)buf, 10);
    if (copy_ret >= 10) {
      if (strncmp(prefix, "A#t#k#F#3#", 10) == 0) {
        buf += 10;
        count -= 10;
        delta = 1;
      }
    ...
    if (buffer) {
      ...
      if (strncmp(pathname, applog_path_prefixes[i],
        ...
        snprintf(timestamp_str, sizeof(timestamp_str),
                 "[AutoLabel %04d-%02d-%02d %02d:%02d:%02d.%09lu %d] %s", tm.tm_year + 1900,
                 tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec,
                 ts.tv_nsec, task_pid_vnr(current), count == 0 ? "\n" : "");
